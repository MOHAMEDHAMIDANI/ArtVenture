    ...
    <MainLay>
        <div class="w-[450px] h-[700px] flex flex-col justify-evenly items-center mx-auto mb-14 border-2 border-black ">
            ...
            <div @click="picUpload.click()" class="w-[100px] h-[100px] border-2 border-black rounded-full overflow-hidden cursor-pointer">
                <input type="file" class="hidden" ref="picUpload">
            ...
            <div class="flex flex-col h-[35%] justify-between items-center">
                <div class="flex items-center flex-col justify-between ">
                    ...
                    <input required v-model="username" id="username" type="text" class="border-b-2 border-black outline-none pl-1 w-[300px] bg-slate-50">
                </div>
                <div class="flex items-center flex-col justify-between ">
...
const picUpload : null | string = ref(null);
const username = ref('')
const password = ref('')
...
const Register = async() =>{
    ...
    try {
        const token  = await axios.post('http://localhost:3000/api/ArtVenture/auth/Register',{username : username.value , email:email.value , password : password.value })
        console.log(token)
    } catch (error) {
        console.log(error)
    }
